@props(['input-class' => '', 'title', 'x-ref' => null])

<div {{ $attributes->class('min-w-46!') }}
    @isset(${'x-ref'})
        x-ref="{{ ${'x-ref'} }}"
...
<div class="w-full flex justify-around items-center px-3 py-2.5 h-11 overflow-hidden input focus-within:focus-input! {{ ${'input-class'} }}"
...
>
    ...
    <input {{ $attributes->only('name')->merge(['type' => 'hidden', 'name' => 'date']) }} :value="getDateValue">
