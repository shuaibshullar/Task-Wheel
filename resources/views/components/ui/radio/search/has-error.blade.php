...
<div class="contents"
    ...
    @endisset
>
<div {{ $attributes->class('group z-100 min-w-0') }}
    x-id="['id']" :id="$id('id')"
    :class="{ 'open': isOpen }"
    x-data="{
        outside: false,
        ...
        input(value, skip = false) {
            ...
            this.valid = Boolean(this.choice);
            if (this.valid)
            {
            ...
            if (! this.isOpen && this.choice && ! skip)
            {
                this.Input.value = this.choice;
                return;
            }
...
>
    ...
    >
        ...
        <div class="contain-layout h-auto w-full overflow-hidden group input group-[&:where(div.open)]:focus-input! group-has-[p.show]:error-input! transition-all text-white {{ ${'input-class'} }}"
